...
ggplot(age_gender_stats, 
  ...
  labs(
    title = "Mean Salary by Age Group and Gender",
    x = "Age Category", 
    y = "Mean Salary (AMD)",
    color = "Gender"
  ) +
...
As the difference in big, let's see how many records are made my males and females.
```{r}
# not null values
total_count <- decoded_data %>%
  filter(!is.na(E14_1)) %>%
  nrow()
...
Let's see how many records we have with filled primary job and secondary
```{r}
